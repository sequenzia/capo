...
5. **Readable.** ~500 lines of Python + a handful of markdown files in `souls/` and `prompts/`.
...
- [ ] Default model is read from `config.toml [models] default`; configurable per-run override is supported.
...
- [ ] Worker calls `amc.send` then `amc.mark_read` after the agent run completes; both are routed through `transport/amc_client.py`.
...
| AMC `ATTACHMENT_TOO_LARGE` | "That attachment is too big for the channel" | Drop attachment, send text only |
...
**Priority**: P0 (Critical)
**Complexity**: Low
**Phase**: 2 (status/output/kill), 2 (list)
...
- [ ] **All `@DBOS.step` calls with external side effects (subprocess spawn, `amc.send`, DB inserts other than within `BEGIN IMMEDIATE` retry helper) generate and persist an idempotency key in the workflow's step state so retries are safe.**
...
- [ ] **Session boundary.** A session ends when the user explicitly starts a new one (slash command `/new` or NL equivalent) — `sessions` table tracks `(thread_id, session_id, started_at, ended_at, compacted_to_message_index)`.
...
- [ ] `approvals` table mirrors workflow state: `(approval_id TEXT PK, user_id, thread_id, action_kind, action_payload_json, status, created_at, resolved_at, choice, reason)`.
- [ ] On guarded action:
...
- [ ] **Main agent model selection.** Capo's main agent uses `config.models.default` on every run unless explicitly overridden by the user.
...
- [ ] **Hard cap behavior.** On crossing hard cap, `capo.run` short-circuits with a "budget exceeded — reply 'override' to continue today" message. Override unlocks the rest of the day.
...
- [ ] Subsystem checks:
  ...
  - `dispatcher` — Workers alive, queue depth < configured max.
...
- [ ] All proactive sends use idempotency keys derived from `(delegation_id, milestone)`.
...
| `capo.tool.<name>` | Tool execution | `tool_name`, `result_status` |
...
| Migrations | Alembic | Conventional, supports both SQLite files via two configs |
...
erDiagram
    ...
    APPROVALS }o--|| DELEGATIONS : "gates (optional)"
...
Identical lifecycle; specifics finalized after spike S-1 (session resume mechanism).
...
Each spike produces a short markdown findings document committed under `specs/spikes/`.
...
| Deliverable | Description | Tasks | Dependencies |
|-------------|-------------|-------|--------------|
| DBOS configuration | Configure DBOS to use `~/.capo/dbos.db`; migrations | `capo/workflows/__init__.py`, Alembic config for dbos.db | Settings |
...
| 6 | CC eventually completes | Workflow runs `summarize_run` + `notify_user`; AMC receives completion message exactly once |
...
Logfire alerts (configured via `mcp__plugin_logfire_logfire__alert_create`):
